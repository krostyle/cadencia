...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 h-16 bg-background/80 backdrop-blur-sm border-t flex items-end pb-2">
      {NAV_ITEMS.map(({ href, icon: Icon, label, primary }) => {
        ...
        return (
          <Link
            ...
            href={href}
            className={cn(
              "flex-1 flex flex-col items-center gap-0.5 rounded-lg transition-colors text-xs",
              active
