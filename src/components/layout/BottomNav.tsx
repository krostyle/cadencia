...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 h-16 bg-background/80 backdrop-blur-sm border-t flex items-center justify-around px-2">
      {NAV_ITEMS.map(({ href, icon: Icon, label, primary }) => {
        ...
        if (primary) {
          return (
            <Link
              ...
              href={href}
              className={cn(
                "flex flex-col items-center justify-center w-14 h-14 rounded-full -mt-6 transition-colors",
                active
        ...
        return (
          <Link
            ...
            href={href}
            className={cn(
              "flex flex-col items-center gap-0.5 py-2 px-3 rounded-lg transition-colors text-xs",
              active
